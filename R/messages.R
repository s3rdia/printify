...
print_message <- function(type,
    ...
    if (!identical(current_session_token, .printify_messages[["last_session"]]) ||
        !identical(current_execution_token, .printify_messages[["last_execution"]])){
...
print_to_console <- function(type,
    ...
    if (!is.list(type)){
        ...
        if (utf8){
            ...
            seperator <- paste0("\n", .printify_messages[["format"]][[paste0(type, "_indent_ansi")]])
            text      <- paste(text, collapse = seperator)

            # Insert special grey message format
            if (type == "grey"){
                text <- paste0("[i][", .printify_messages[["format"]][["time_color"]], " ", text, "][/i]")
            }

            # Insert formatting
            text_orig <- convert_square_brackets(text, ...)
            text      <- paste(.printify_messages[["format"]][[paste0(type, "_ansi")]], text_orig, sep = "")
        }
        else{
            # First format all line breaks so that the next text lines are indented
            seperator <- paste0("\n", .printify_messages[["format"]][[paste0(type, "_indent_pt")]])
            text      <- paste(text, collapse = seperator)
            ...
            # Insert formatting
            text_orig <- convert_square_brackets(text, ...)
            text      <- paste(.printify_messages[["format"]][[paste0(type, "_pt")]], text_orig, sep = "")
        }
    }
    # If custom message
    else{
    ...
    .printify_messages[["last_message"]] <- text
...
print_start_message <- function(current_time = Sys.time(),
    ...
    if (!always_print && is_message_suppressed()){
        ...
        if (!identical(current_session_token, .printify_messages[["last_session"]]) ||
            ...
            reset_messages()
            .printify_messages[["last_session"]]   <- current_session_token
            .printify_messages[["last_execution"]] <- current_execution_token
        ...
        .printify_messages[["start_time"]] <- current_time
        start_timer()
...
print_closing <- function(time_threshold = 2,
    ...
    if (!always_print && is_message_suppressed()){
        ...
        if (is_stack() && (!identical(current_session_token, .printify_messages[["last_session"]]) ||
                           !identical(current_execution_token, .printify_messages[["last_execution"]]))){
            if (!is_start_fake()){
                suppressed <- TRUE
            }
        ...
        if (!is_stack()){
            if (length(.printify_messages[["stack"]]) == 0){
                .printify_messages[["stack"]] <- list(list(type = "START", suppressed = TRUE, text = "FAKE"))
            }
            else{
                .printify_messages[["stack"]] <- c(list(list(type = "START", suppressed = TRUE, text = "FAKE")), .printify_messages[["stack"]])
            }
...
print_step <- function(type,
    ...
    else if (is.null(.printify_messages[["timer"]])){
...
get_message_stack <- function(as_data_frame = FALSE){
    # Get stack as is
    if (!as_data_frame){
        .printify_messages[["stack"]]
    }
    # Convert to data frame without the whole call stack
    else{
        stack_df <- do.call(rbind, lapply(.printify_messages[["stack"]], function(message){
...
print_stack_as_messages <- function(type = NULL){
    for (message in .printify_messages[["stack"]]){
...
#' @noRd
is_stack <- function(){
    length(.printify_messages[["stack"]]) > 0 && .printify_messages[["stack"]][[1]][["type"]] == "START"
}
...
is_full_stack <- function(){
    is_stack() &&
        .printify_messages[["stack"]][[length(.printify_messages[["stack"]])]][["type"]] == "END" &&
        !.printify_messages[["stack"]][[length(.printify_messages[["stack"]])]][["suppressed"]] &&
        .printify_messages[["stack"]][[1]][["caller"]] == .printify_messages[["stack"]][[length(.printify_messages[["stack"]])]][["caller"]] &&
        .printify_messages[["stack"]][[1]][["depth"]] == .printify_messages[["stack"]][[length(.printify_messages[["stack"]])]][["depth"]]
}
...
is_single_non_starting_message <- function(){
    length(.printify_messages[["stack"]]) == 1 && .printify_messages[["stack"]][[1]][["type"]] != "START"
}
...
is_starting_message_suppressed <- function(){
    length(.printify_messages[["stack"]]) >= 1 && .printify_messages[["stack"]][[1]][["suppressed"]]
}
...
is_last_message_error <- function(){
    length(.printify_messages[["stack"]]) >= 1 && .printify_messages[["stack"]][[length(.printify_messages[["stack"]])]][["type"]] == "ERROR"
}
...
is_start_fake <- function(){
    length(.printify_messages[["stack"]]) >= 1 &&
        .printify_messages[["stack"]][[1]][["type"]] == "START" &&
        .printify_messages[["stack"]][[1]][["text"]] == "FAKE"
}
...
is_new_line_necessary <- function(){
    !is.null(.printify_messages[["last_message"]]) && !.printify_messages[["stack"]][[length(.printify_messages[["stack"]])]][["new_line"]]
}
...
is_time_stamp_possible <- function(){
    !is.null(.printify_messages[["timer"]]) && !is.null(.printify_messages[["last_message"]])
}
...
is_timer_running <- function(){
    !is.null(.printify_messages[["timer"]])
}
...
awaits_time_print <- function(){
    length(.printify_messages[["stack"]]) > 0 && .printify_messages[["stack"]][[length(.printify_messages[["stack"]])]][["print_time"]]
}
...
get_last_active_message <- function(){
    last_entry <- length(.printify_messages[["stack"]])
    ...
    for (i in last_entry:1){
        entry <- .printify_messages[["stack"]][[i]]
...
stop_timer <- function(){
    ...
    invisible(.printify_messages[["timer"]])
}
...
name_stack_messages <- function(){
    names(.printify_messages[["stack"]]) <- vapply(.printify_messages[["stack"]], function(message){
    ...
    invisible(.printify_messages[["stack"]])
}
...
print_time_stamp <- function(utf8 = .printify_messages[["format"]][["utf8"]]){
    if (is_time_stamp_possible()){
        ...
        if (is_time_stamp_allowed(last_message)){
            # Calculate time difference between provided starting time and current time
            formatted_time <- transform_time(.printify_messages[["timer"]])
            start_timer()
